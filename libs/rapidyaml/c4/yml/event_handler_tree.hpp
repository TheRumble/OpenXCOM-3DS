...
public:
    ...
    static NodeData _val2key_(NodeData const& C4_RESTRICT d) noexcept
    {
        ...
        r.m_type = d.m_type;
        static_assert(
            (static_cast<type_bits>(_VALMASK) >> 1u) ==
                static_cast<type_bits>(_KEYMASK),
            "required for this function to work");
        static_assert(
            (static_cast<type_bits>(VAL_STYLE) >> 1u) ==
                static_cast<type_bits>(KEY_STYLE),
            "required for this function to work");
        r.m_type.type = static_cast<NodeType_e>(
            static_cast<type_bits>(
                d.m_type.type & (_VALMASK | VAL_STYLE)) >> 1u);
