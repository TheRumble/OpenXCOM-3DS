...
{
	/*
	 * Grouped TextButtons handle left presses internally even when
	 * they do not have a separate click or press callback. This is
	 * how radio-button groups such as the New Game difficulty choices
	 * select their active item.
	 *
	 * ToggleTextButton also uses an internal fake group, so this makes
	 * those controls correctly report that they accept left presses.
	 */
	if (_group != 0 && button == SDL_BUTTON_LEFT)
	{
		return true;
	}
