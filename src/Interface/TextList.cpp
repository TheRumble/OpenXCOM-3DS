...
/**
 * Returns whether the list accepts row selection.
 */
...
/**
 * Enables or disables one logical row for selection.
 */
void TextList::setRowSelectable(size_t row, bool selectable)
{
	if (row < _rowSelectable.size())
	{
		_rowSelectable[row] = selectable;
	}
}

/**
 * Returns whether one logical row can be selected.
 */
bool TextList::isRowSelectable(size_t row) const
{
	return row < _rowSelectable.size() &&
		_rowSelectable[row];
}
