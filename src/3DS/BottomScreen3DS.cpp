...
NativeRect3DS battlescapeControlNativeRect3DS(
        int controlIndex)
{
        const ControlTarget &control =
                BATTLESCAPE_CONTROLS[controlIndex];

        /*
         * The first two controls are the stacked hand slots.
         */
        if (controlIndex < 2)
        {
                if (isTftdMaster3DS())
                {
                        /*
                         * The cleaned TFTD hand-slot artwork occupies
                         * x=257..320 after the connector crop.
                         */
                        return
                        {
                                257,
        ...
        /*
         * The fourteen main command buttons use source x=48..272,
         * y=0..32 and destination x=0..320, y=0..46.
         */
        if (controlIndex >= 2 &&
                controlIndex <= 15)
        {
        ...
        /*
         * The TFTD reserve/status strip uses source x=43..272,
         * y=32..56 and destination x=0..320, y=46..80.
         *
         * UFO Defense retains the original x=48..272, y=0..56
         * mapping.
         */
        if (isTftdMaster3DS())
        {
...
NativeRect3DS battlescapeStatsNativeRect3DS()
{
        if (isTftdMaster3DS())
        {
                return
        ...
        int bottomHeight)
{
        ...
        constexpr int topStripSourceLeft = 48;
        constexpr int topStripSourceRight = 272;
        constexpr int topStripSourceWidth =
                topStripSourceRight - topStripSourceLeft; // 224

        constexpr int topStripSourceHeight = 56;
        constexpr int topStripTargetWidth = 320;
        constexpr int topStripTargetHeight = 80;
