...
public:
	...
	/// 3DS helper: snaps cursor to a visible battlescape unit.
	bool snapVisibleUnit3DS(
			int directionX,
			int directionY,
			int currentX,
			int currentY,
			int &targetX,
			int &targetY) const;
	/// 3DS helper: sets C-stick camera panning speed.
	void setCStickPan3DS(int dx, int dy);
...
#ifdef NINTENDO_3DS
	/// Gets the active Battlescape state for lower-screen rendering.
	static BattlescapeState *getActive3DS();
	/// Builds the live original Battlescape interface strip for the 3DS lower panel.
	SDL_Surface *getBottomPanelSource3DS();
	/// Shows or clears the relocated TFTD Zero TUs tooltip.
	void setZeroTUsHover3DS(bool active);
	/// Holds or releases the relocated TFTD Zero TUs visual state.
	void setZeroTUsPressed3DS(bool pressed);
#endif
