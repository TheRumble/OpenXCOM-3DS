...
#ifdef __3DS__
	_placementPreview3DS = new Surface(width, height, 0, 0);
#endif
...
#ifdef __3DS__
	_placementPreview3DS->setPalette(
		colors,
		firstcolor,
		ncolors);
#endif
...
/**
 * Updates the 3DS placement footprint for the currently carried item.
 *
 * Green means the current anchor appears placeable. Red means that
 * the current target is blocked or otherwise invalid.
 *
 * Inventory::mouseClick remains authoritative for the actual action.
 */
void Inventory::updatePlacementPreview3DS()
{
	_placementPreview3DS->clear();

	if (!_selUnit || !_selItem || !_selItem->getRules())
	{
		return;
	}

	const RuleItem *rules = _selItem->getRules();

	const int itemWidth =
		std::max(
			1,
			rules->getInventoryWidth());

	const int itemHeight =
		std::max(
			1,
			rules->getInventoryHeight());

	/*
	 * Match the carried-item anchor calculation used by mouseClick().
	 */
	int slotX =
		_selection->getX() +
		(RuleInventory::HAND_W - itemWidth) *
			RuleInventory::SLOT_W / 2 +
		RuleInventory::SLOT_W / 2;

	int slotY =
		_selection->getY() +
		(RuleInventory::HAND_H - itemHeight) *
	...
	/*
	 * Preserve screen-local coordinates before adding the ground-page
	 * offset used internally by OXCE.
	 */
	const int displaySlotX = slotX;
	const int displaySlotY = slotY;
	...
	const bool sectionAllowed =
		rules->
			canBePlacedIntoInventorySection(
				slot);

	const bool footprintFits =
		slot->fitItemInSlot(
			rules,
			slotX,
			slotY);
	...
	{
		/*
		 * Normal empty placement, restoring over the selected item,
		 * or stacking an equivalent ground item.
		 */
		if (!targetItem ||
			targetItem == _selItem ||
			canStack)
		{
			valid =
				(footprintFits && !overlaps) ||
				canStack;
		}
		/*
		 * Compatible ammunition over an unloaded weapon.
		 */
		else if (targetItem->isWeaponWithAmmo())
		{
			const int ammoSlot =
				targetItem->
					getRules()->
					getSlotForAmmo(rules);
	...
	int previewX = slot->getX();
	int previewY = slot->getY();
	...
	footprint.x = previewX;
	footprint.y = previewY;
	footprint.w = previewWidth;
	footprint.h = previewHeight;
	...
	_placementPreview3DS->drawRect(
		&footprint,
		color);

	/*
	 * Clear the center so only a one-pixel outline remains.
	 */
	if (footprint.w > 2 &&
		footprint.h > 2)
	{
...
#ifdef __3DS__
	_placementPreview3DS->blitNShade(
		this,
		0,
		0);
#endif
...
bool Inventory::quickMoveItemAt3DS(int absoluteX, int absoluteY)
{
	if (_selUnit == 0)
	{
		return false;
	...
	BattleItem *item = _mouseOverItem;
	const RuleInventory *slot = item ? item->getSlot() : nullptr;

	if (item == 0)
	{
		int x = absoluteX - getX();
		int y = absoluteY - getY();

		RuleInventory *cursorSlot = getSlotInPosition(&x, &y);
		if (cursorSlot == 0)
		{
			return false;
		}

		if (cursorSlot->getType() == INV_GROUND)
		{
			x += _groundOffset;
		}

		item = _selUnit->getItem(cursorSlot, x, y);
		slot = cursorSlot;
	}

	if (item == 0 || item->getRules()->isFixed() || slot == 0)
	{
		return false;
	}

	const RuleInventory *originalSlot = item->getSlot();
	const int originalX = item->getSlotX();
	const int originalY = item->getSlotY();
	const bool originalWasGround =
			originalSlot &&
			originalSlot->getType() == INV_GROUND;
	...
	if (slot->getType() == INV_GROUND)
	{
		...
		if (!placed)
		{
			...
			if (item->getRules()->getDefaultInventorySlot() &&
					item->getRules()->getDefaultInventorySlot()->getType() != INV_GROUND)
			{
				newSlot = item->getRules()->getDefaultInventorySlot();
				placed = fitItem(newSlot, item, warning);
			}
	...
	else if (originalWasGround)
	{
		/*
		 * Keep X usable on ground stacks: after one item moves,
		 * retarget whatever remains in the same ground slot. If the
		 * stack is empty, this naturally clears the hover item.
		 */
		setMouseOverItem(
				_selUnit->getItem(
						originalSlot,
