...
{
	/*
	 * The original popup filled the 144-pixel map area above the
	 * Battlescape command bar. The 3DS displays the map through the
	 * former 56-pixel bar area, so extend and recenter the popup.
	 */
#ifdef NINTENDO_3DS
	...
	_window = new Window(
		...
		0,
		0);
	_txtInEntrance = new Text(
		304, 17, 16, 20 + contentOffsetY);
	_txtInExit = new Text(
		304, 17, 16, 40 + contentOffsetY);
	_txtOutside = new Text(
		304, 17, 16, 60 + contentOffsetY);
	_txtAbort = new Text(
