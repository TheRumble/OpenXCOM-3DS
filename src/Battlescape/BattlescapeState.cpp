...
namespace
{
BattlescapeState *activeBattlescapeState3DS = nullptr;
Surface::UniqueBufferPtr bottomPanelSourceBuffer3DS;
Surface::UniqueSurfacePtr bottomPanelSource3DS;
}
#endif
...
{
#ifdef NINTENDO_3DS
	activeBattlescapeState3DS = this;
#endif
...
#ifdef NINTENDO_3DS
	const int visibleMapHeight = screenHeight;
#else
#ifdef NINTENDO_3DS
	const int visibleMapHeight = screenHeight;
#else
	...
	for (int i = 0; i < VISIBLE_MAX; ++i)
	{
#ifdef NINTENDO_3DS
		/*
		 * Keep visible-unit/enemy center markers on their normal
		 * right-side X position, but move them down into the freed
		 * bottom strip of the top screen.
		 */
		const int visibleUnitButtonY =
			screenHeight - 18 - (i * 13);
#else
		const int visibleUnitButtonY =
			y + visibleUnitY - (i * 13);
#endif
		...
		_numVisibleUnit[i] =
			new NumberText(
				15,
				12,
				_btnVisibleUnit[i]->getX() + 6,
				_btnVisibleUnit[i]->getY() + 4);
	}
	...
	_txtTooltip = new Text(300, 10, x + 2, screenHeight - 10);
...
#ifdef NINTENDO_3DS
	auto skipMainBlit3DS =
		[](Surface *surface)
		{
			if (surface)
			{
				surface->setSkipMainBlit3DS(true);
			}
		};

	/*
	 * The Battlescape command strip is rendered on the lower screen
	 * from these live UI objects. Keep them interactive for synthetic
	 * bottom-screen input, but do not duplicate-blit them into the
	 * normal top-screen frame.
	 */
	skipMainBlit3DS(_icons);

	skipMainBlit3DS(_rank);
	skipMainBlit3DS(_rankTiny);

	skipMainBlit3DS(_btnUnitUp);
	skipMainBlit3DS(_btnUnitDown);
	skipMainBlit3DS(_btnMapUp);
	skipMainBlit3DS(_btnMapDown);
	skipMainBlit3DS(_btnShowMap);
	skipMainBlit3DS(_btnKneel);
	skipMainBlit3DS(_btnInventory);
	skipMainBlit3DS(_btnCenter);
	skipMainBlit3DS(_btnNextSoldier);
	skipMainBlit3DS(_btnNextStop);
	skipMainBlit3DS(_btnShowLayers);
	skipMainBlit3DS(_btnHelp);
	skipMainBlit3DS(_btnEndTurn);
	skipMainBlit3DS(_btnAbort);

	skipMainBlit3DS(_btnStats);
	skipMainBlit3DS(_txtName);
	skipMainBlit3DS(_numLayers);
	skipMainBlit3DS(_numTimeUnits);
	skipMainBlit3DS(_numEnergy);
	skipMainBlit3DS(_numHealth);
	skipMainBlit3DS(_numMorale);
	skipMainBlit3DS(_barTimeUnits);
	skipMainBlit3DS(_barEnergy);
	skipMainBlit3DS(_barHealth);
	skipMainBlit3DS(_barMorale);

	if (_manaBarVisible)
	{
		skipMainBlit3DS(_barMana);
	}

	skipMainBlit3DS(_btnReserveNone);
	skipMainBlit3DS(_btnReserveSnap);
	skipMainBlit3DS(_btnReserveAimed);
	skipMainBlit3DS(_btnReserveAuto);
	skipMainBlit3DS(_btnReserveKneel);
	skipMainBlit3DS(_btnZeroTUs);

	skipMainBlit3DS(_btnLeftHandItem);
	skipMainBlit3DS(_btnRightHandItem);

	for (auto *text : _numAmmoLeft)
	{
		skipMainBlit3DS(text);
	}

	for (auto *text : _numAmmoRight)
	{
		skipMainBlit3DS(text);
	}

	for (auto *text : _numMedikitLeft)
	{
		skipMainBlit3DS(text);
	}

	for (auto *text : _numMedikitRight)
	{
		skipMainBlit3DS(text);
	}

	skipMainBlit3DS(_numTwoHandedIndicatorLeft);
	skipMainBlit3DS(_numTwoHandedIndicatorRight);
	skipMainBlit3DS(_warning);
#endif
...
{
#ifdef NINTENDO_3DS
	if (activeBattlescapeState3DS == this)
	{
		activeBattlescapeState3DS = nullptr;
		bottomPanelSource3DS = nullptr;
		bottomPanelSourceBuffer3DS = nullptr;
	}
#endif
...
#ifdef NINTENDO_3DS
BattlescapeState *BattlescapeState::getActive3DS()
{
	return activeBattlescapeState3DS;
}

SDL_Surface *BattlescapeState::getBottomPanelSource3DS()
{
	const int sourceWidth = Options::baseXResolution;
	const int sourceHeight = Options::baseYResolution;

	if (sourceWidth <= 0 ||
		sourceHeight <= 0 ||
		!_icons ||
		!static_cast<bool>(*_icons))
	{
		return nullptr;
	}

	if (!bottomPanelSource3DS ||
		bottomPanelSource3DS->w != sourceWidth ||
		bottomPanelSource3DS->h != sourceHeight)
	{
		auto pair =
			Surface::NewPair8Bit(
				sourceWidth,
		...
		bottomPanelSourceBuffer3DS =
			std::move(pair.first);

		bottomPanelSource3DS =
	...
	SDL_Surface *iconsSurface =
		_icons->getSurface();

	if (iconsSurface &&
		iconsSurface->format &&
		iconsSurface->format->palette &&
		bottomPanelSource3DS->format &&
		bottomPanelSource3DS->format->palette)
	{
		SDL_SetColors(
			bottomPanelSource3DS.get(),
			iconsSurface->format->palette->colors,
			0,
			iconsSurface->format->palette->ncolors);
	}

	Surface::CleanSdlSurface(
		bottomPanelSource3DS.get());
	...
	/*
	 * Compose the same original logical interface strip, but into a
	 * private source frame instead of depending on the top-screen frame.
	 * The bottom-screen layout code can then crop/rearrange this source.
	 */
	blit(_icons);
	...
	blit(_btnReserveNone);
	blit(_btnReserveSnap);
	blit(_btnReserveAimed);
	blit(_btnReserveAuto);
	blit(_btnReserveKneel);
	blit(_btnZeroTUs);

	blit(_btnLeftHandItem);
	blit(_btnRightHandItem);
	...
	blit(_numTwoHandedIndicatorLeft);
	blit(_numTwoHandedIndicatorRight);

	blit(_warning);

	return bottomPanelSource3DS.get();
}


void BattlescapeState::setZeroTUsHover3DS(bool active)
{
	...
	{
		if (allowButtons() &&
			Options::battleTooltips)
		{
			_currentTooltip = tooltip;

			_txtTooltip->setColor(
				_tooltipDefaultColor);
	...
	if (_currentTooltip == tooltip)
	{
		...
		_txtTooltip->setText("");
		_currentTooltip.clear();
	}
}


void BattlescapeState::setZeroTUsPressed3DS(
	bool pressed)
{
	if (_btnZeroTUs)
	{
		_btnZeroTUs->toggle(pressed);

		/*
		 * The lower Battlescape panel is cached. Rebuild it
		 * immediately so even a quick Y press captures both
		 * the pressed and released button frames.
		 */
		BottomScreen3DS::requestPanelRefresh();
	}
}
...
void BattlescapeState::setCStickPan3DS(int dx, int dy)
{
	if (!_map || !_map->getCamera())
	{
		return;
	}
	...
	_map->getCamera()->setKeyScroll3DS(dx, dy);
}

bool BattlescapeState::snapVisibleUnit3DS(
		int directionX,
		int directionY,
		int currentX,
		int currentY,
		...
		int &targetY) const
{
	...
	const Position mapOffset = camera->getMapOffset();

	const int mapWidth = _map->getWidth();
	#ifdef NINTENDO_3DS
        /*
         * The old command bar is no longer part of the top-screen
         * Battlescape layout. Top-screen snapping must include the
         * freed lower strip where visible-unit markers can now live.
         */
	...
	#ifdef NINTENDO_3DS
	/*
	 * 3DS top button snap targets:
	 *
	 * The Battlescape top snap pass is not only unit sprites.
	 * The visible-unit marker buttons and launch/special buttons
	 * are real top-screen controls too, so expose their centers
	 * as D-pad snap candidates.
	 */
	auto absSnap3DS = [](int value)
	{
	        return value < 0 ? -value : value;
	};

	auto considerTopSnapRect3DS =
	        [&](int left,
	                int top,
	                int right,
	                int bottom)
	{
	        ...
	        if ((directionX < 0 && deltaX >= 0) ||
	                (directionX > 0 && deltaX <= 0) ||
	                (directionY < 0 && deltaY >= 0) ||
	                (directionY > 0 && deltaY <= 0))
	        {
	                return;
	        }

	        if (absSnap3DS(deltaX) <= 1 &&
	                absSnap3DS(deltaY) <= 1)
	        {
	                return;
	        }
	...
	auto considerTopSnapSurface3DS =
	        [&](Surface *surface)
	{
	        if (!surface ||
	                !surface->getVisible())
	...
	for (int i = 0; i < VISIBLE_MAX; ++i)
	{
	        if (_visibleUnit[i])
	        {
	                considerTopSnapSurface3DS(
	                        _btnVisibleUnit[i]);
	        }
	}

	considerTopSnapSurface3DS(_btnLaunch);
	considerTopSnapSurface3DS(_btnPsi);
	considerTopSnapSurface3DS(_btnSpecial);
	considerTopSnapSurface3DS(_btnSkills);
	#endif


	auto considerUnit = [&](BattleUnit *unit)
	{
		if (!unit || unit->isOut())
		{
			return;
		...
		const bool playerUnit =
				unit->getFaction() == FACTION_PLAYER &&
				unit->getStatus() != STATUS_DEAD;

		const bool visibleNonPlayerUnit =
				unit->getFaction() != FACTION_PLAYER &&
				unit->getStatus() != STATUS_DEAD &&
				(unit->getVisible() || _save->getDebugMode());

		if (!playerUnit && !visibleNonPlayerUnit)
		{
			return;
		}
		...
		screenPosition += mapOffset;

#ifdef NINTENDO_3DS
		/*
		 * 3DS friendly snap filter:
		 *
		 * Friendly units should only be D-pad snap targets when
		 * they are actually usable on the current top-screen view.
		 * This prevents snapping to soldiers hidden on another
		 * level or outside the visible camera area.
		 *
		 * Visible non-player units are intentionally not filtered
		 * here, because spotted aliens should remain reachable.
		 */
		if (playerUnit)
		{
		        const bool onCurrentLayer =
		                camera->getShowAllLayers() ||
		                unit->getPosition().z ==
		                        camera->getViewLevel();

		        const bool insideTopScreen =
		                screenPosition.x >= 0 &&
		                screenPosition.x < mapWidth &&
		                screenPosition.y >= 0 &&
		                screenPosition.y < mapHeight;

		        if (!onCurrentLayer || !insideTopScreen)
		        {
		                return;
		        }
		}
		...
		/*
		 * Aim near the occupied tile center. This is intentionally
		 * unit-only snapping, not tile-by-tile map snapping.
		 */
		const int candidateX = screenPosition.x + 16;
		const int candidateY = screenPosition.y + 16;

		if (candidateX < 0 ||
				candidateX >= mapWidth ||
				candidateY < 0 ||
				candidateY >= mapHeight)
		{
			return;
		}

		const int deltaX = candidateX - currentX;
		const int deltaY = candidateY - currentY;
		...
		/*
		 * Prefer the nearest target in the requested direction, with
		 * a penalty for being far off-axis.
		 */
		const int score =
				forward * forward +
				lateral * lateral * 3;
