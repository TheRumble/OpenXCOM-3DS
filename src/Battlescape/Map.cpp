...
#include "../fmath.h"
#ifdef NINTENDO_3DS
#include "../3DS/RenderProfile3DS.h"
#endif
...
void Map::draw()
{
...
#ifdef NINTENDO_3DS
	RenderProfile3DS::beginMapFrame(
		RenderProfile3DS::consumeSelectorRedraw());
#endif
...
#ifdef NINTENDO_3DS
	/*
	 * OXCE_3DS_FAST_MAP_CLEAR
	 *
	 * The Battlescape map is an 8-bit surface filled with one
	 * palette index. Avoid the generic per-pixel shader controller.
	 */
	const Uint8 backgroundColor3DS =
		static_cast<Uint8>(
			Palette::blockOffset(0) + _bgColor);
	...
	{
#ifdef NINTENDO_3DS
		const u64 profileTerrainStart3DS =
			RenderProfile3DS::now();
#endif

		drawTerrain(this);

#ifdef NINTENDO_3DS
		RenderProfile3DS::setTerrainTicks(
			RenderProfile3DS::now() -
				profileTerrainStart3DS);
#endif
	}
	else
	{
...
void Map::drawTerrain(Surface *surface)
{
	...
	auto *cursorSet =
		_game->getMod()->getSurfaceSet("CURSOR.PCK");
	...
	auto tileMayNeedDrawUnitCall3DS =
		[&](Tile *candidateUnitTile,
			Tile *candidateCurrTile,
			BattleUnit *candidateMovingUnit)
		{
			if (!candidateUnitTile)
			{
				return false;
			}

			BattleUnit *candidateUnit =
				candidateUnitTile->getUnit();

			if (!candidateUnit)
			{
				const Position candidatePosition =
					candidateUnitTile->getPosition();

				if (candidatePosition.z > 0 &&
					candidateUnitTile->hasNoFloor(_save) &&
					!candidateUnitTile->getMapData(O_OBJECT))
				{
					Tile *belowTile =
						_save->getBelowTile(candidateUnitTile);

					if (belowTile)
					{
						BattleUnit *belowUnit =
							belowTile->getUnit();

						if (belowUnit &&
							belowUnit->getHeight() +
							belowUnit->getFloatHeight() -
							belowTile->getTerrainLevel() > 0)
						{
							candidateUnit = belowUnit;
						}
					}
				}
			}
			...
			{
				/*
				 * Preserve drawUnit()'s special moving-unit path:
				 * when a moving unit is on the tile above a no-floor
				 * tile, drawUnit resolves that internally.
				 */
				return candidateMovingUnit &&
					candidateUnitTile == candidateCurrTile;
			}

			if (!(candidateUnit->getVisible() ||
				_save->getDebugMode()))
			{
				return false;
			}

			if (candidateUnit->getStatus() == STATUS_WALKING ||
				candidateUnit->getStatus() == STATUS_FLYING)
			{
				return true;
			}

			/*
			 * Ordinary stationary units only draw from their own tile.
			 */
			return candidateUnitTile == candidateCurrTile;
		};

	auto drawUnitIfCandidate =
		[&](Tile *candidateUnitTile,
			Tile *candidateCurrTile,
			Position candidateScreenPosition,
			bool candidateTopLayer,
			BattleUnit *candidateMovingUnit)
		{
			if (!tileMayNeedDrawUnitCall3DS(
				candidateUnitTile,
				candidateCurrTile,
				candidateMovingUnit))
			{
	...
	const int surfaceWidthWithMargin =
		surface->getWidth() + _spriteWidth;
	const int surfaceHeightWithMargin =
		surface->getHeight() + _spriteHeight;
	const bool mouseOverIcons =
	...
	for (int itZ = beginZ; itZ <= endZ; itZ++)
	{
		...
		for (int itY = beginY; itY < endY; itY++)
		{
			...
			for (int itX = beginX; itX < endX; itX++, mapPosition.x++, tile++)
			{
#ifdef NINTENDO_3DS
				/*
				 * OXCE_3DS_DIRECT_TILE_COORDINATES
				 *
				 * Camera::convertMapToScreen is a small linear calculation,
				 * but calling it once for every visible tile adds overhead.
				 */
				screenPosition.x =
					mapPosition.x * (_spriteWidth / 2) -
					mapPosition.y * (_spriteWidth / 2);

				screenPosition.y =
					mapPosition.x * (_spriteWidth / 4) +
					mapPosition.y * (_spriteWidth / 4) -
					mapPosition.z *
						((_spriteHeight +
							_spriteWidth / 4) / 2);
				...
				if (screenPosition.x > -_spriteWidth && screenPosition.x < surfaceWidthWithMargin &&
					screenPosition.y > -_spriteHeight && screenPosition.y < surfaceHeightWithMargin )
				{
					...
					// Draw cursor back
					if (_cursorType != CT_NONE && _selectorX > itX - _cursorSize && _selectorY > itY - _cursorSize && _selectorX < itX+1 && _selectorY < itY+1 && !mouseOverIcons)
					{
						if (viewLevel == itZ)
						{
							...
							tmpSurface =
								cursorSet ?
									cursorSet->getFrame(frameNumber) :
									nullptr;
							if (tmpSurface)
							{
					...
					if (isUnitMovingNearby)
					{
						...
						{
							drawUnitIfCandidate(_save->getTile(mapPosition + backPos[b]), tile, screenPosition, topLayer, movingUnit);
						}
					}
					...
					drawUnitIfCandidate(tile, tile, screenPosition, topLayer, isUnitMovingNearby ? movingUnit : nullptr);
					...
					if (isUnitMovingNearby)
					{
						...
						{
							drawUnitIfCandidate(_save->getTile(mapPosition + frontPos[f]), tile, screenPosition, topLayer, isUnitMovingNearby ? movingUnit : nullptr);
						}
					}
...
#else
	/*
	 * On 3DS, the Circle Pad/touch trackpad should be able to rest
	 * on the screen edge without auto-scrolling the Battlescape.
	 * Camera panning is handled deliberately by the C-stick instead.
	 */
	_camera->stopMouseScrolling();
#endif
...
void Map::setSelectorPosition(int mx, int my)
{
	...
	{
#ifdef NINTENDO_3DS
		RenderProfile3DS::markSelectorRedraw();
#endif
		_redraw = true;
	}
}
