...
GeoscapeState::GeoscapeState() : _pause(false), _zoomInEffectDone(false), _zoomOutEffectDone(false), _minimizedDogfights(0), _slowdownCounter(0)
{
	...
	#ifdef NINTENDO_3DS
        /*
         * Use the full logical top-screen width. The original sidebar
         * is layered over the right side only for bottom-screen capture.
         */
        _globe = new Globe(
                _game,
                screenWidth / 2,
...
#ifdef NINTENDO_3DS
	/*
	 * Persistent surfaces used by the custom top-screen
	 * compositor. They are deliberately not added to State.
	 */
	_top3DS = new Surface(
	        screenWidth,
	        screenHeight,
	...
	/*
	 * These are complete rectangular images, not sprites.
	 * Palette index zero must therefore overwrite old pixels.
	 */
	SDL_SetColorKey(
	        _top3DS->getSurface(),
	        0,
...
#ifdef NINTENDO_3DS
	/*
	 * setInterface() has now loaded the geoscape palette.
	 * The private compositor surfaces are not part of State,
	 * so give them that palette explicitly.
	 */
	_top3DS->setPalette(_palette);
	_globe3DS->setPalette(_palette);
#endif

	add(_bg);
#ifdef NINTENDO_3DS
	/*
	 * The right sidebar remains in the normal logical frame
	 * for the lower-screen capture, but the underlying globe
	 * now spans the complete 320-pixel top viewport.
	 */
	add(_globe);
	add(_sideLine);
	add(_sidebar);
...
#ifdef NINTENDO_3DS
        /*
         * add() has already initialized every object and registered
         * ownership through preAdd(). The 3DS top geoscape contains
         * only the background and full-width globe.
         */
        for (auto i = _surfaces.begin();
                i != _surfaces.end();)
        {
                Surface *child = *i;
...
#ifdef NINTENDO_3DS
/**
 * Replaces the captured logical geoscape image with the centered
 * 3DS top-screen layout.
 *
 * BottomScreen3DS captures the complete original frame first, so the
 * hidden sidebar, clock and button states remain available below.
 */
void GeoscapeState::renderTop3DS(SDL_Surface *surface)
{
        if (!surface ||
                !_bg ||
                !_globe)
        ...
        /*
         * Replace the complete logical screen with the centered region
         * of the real 768x600 ALTGEOBORD artwork.
         */
        ...
        /*
         * This is now the actual full-width 320x200 globe surface,
         * rather than a rectangular copy of the old 256x200 viewport.
         */
        _globe->blit(surface);

        /*
         * GeoscapeState::blit() drew these before this top replacement,
         * so restore active dogfight windows afterward.
         */
        for (auto *dfs : _dogfights)
        {
                dfs->blit();
...
bool GeoscapeState::renderSidebar3DS(
        SDL_Surface *surface)
{
        if (!surface ||
                surface->w < 64 ||
        ...
        /*
         * Preserve original logical coordinates for input and game
         * logic. Translate only during this bottom-screen draw.
         */
        for (Surface *child : _bottomSurfaces3DS)
        {
                if (!child ||
                        child == _sideLine)
                {
...
#ifdef NINTENDO_3DS
        SDL_Surface *screen =
                _game->getScreen()->getSurface();

        /*
         * Sidebar objects remain active for input and lower-screen
         * rendering, but never appear on the visible top screen.
         */
        _bg->blit(screen);
        _globe->blit(screen);
#else
...
void GeoscapeState::handle(Action *action)
{
        if (_dogfights.size() == _minimizedDogfights)
        {
                ...
                const bool bottomMouseEvent =
                        mouseButtonEvent &&
                        details->button.which ==
                                BottomScreen3DS::
                                BOTTOM_CONTROL_EVENT_WHICH;

                auto handleBottomSurfaces =
                        [this, action]()
                {
                        for (auto i =
                                        _bottomSurfaces3DS.rbegin();
                                i !=
                                        _bottomSurfaces3DS.rend();
                                ++i)
                        {
                ...
                {
                        /*
                         * A lower-screen control event reaches only
                         * the bottom-owned geoscape controls.
                         */
                        handleBottomSurfaces();
                }
                else
                {
                        /*
                         * Ordinary top-screen events reach only the
                         * background and globe collection.
                         */
                        State::handle(action);

                        /*
                         * Preserve keyboard shortcuts registered on
                         * geoscape command and time-speed buttons.
                         */
                        if (details &&
                                (details->type ==
                                        SDL_KEYDOWN ||
...
#ifdef NINTENDO_3DS
/**
 * Checks whether an expanded interception window currently owns
 * directional navigation.
 */
bool GeoscapeState::canNavigateDogfightControls3DS() const
{
	if (!_popups.empty() ||
			!_dogfightsToBeStarted.empty())
	{
		return false;
	}

	for (DogfightState *dogfight : _dogfights)
	{
		if (dogfight &&
				!dogfight->isMinimized() &&
				dogfight->
						hasNavigationControls3DS())
		{
			return true;
		}
	}
...
/**
 * Finds the best spatial target across every expanded interception
 * window and returns physical top-screen cursor coordinates.
 */
bool GeoscapeState::snapToDogfightControl3DS(
		int directionX,
		int directionY,
		int cursorX,
		int cursorY,
		...
		int &targetY) const
{
	if (!canNavigateDogfightControls3DS() ||
			!_game ||
			!_game->getScreen() ||
			(directionX == 0 &&
			 directionY == 0))
	...
	for (DogfightState *dogfight : _dogfights)
	{
		...
		int pointX = logicalCursorX;
		int pointY = logicalCursorY;
		long pointScore = LONG_MAX;

		if (dogfight->
				findNavigationTarget3DS(
						0,
						0,
						logicalCursorX,
						logicalCursorY,
	...
	bool found = false;
	int bestX = logicalCursorX;
	int bestY = logicalCursorY;
	...
	for (DogfightState *dogfight : _dogfights)
	{
		...
		int candidateX = logicalCursorX;
		int candidateY = logicalCursorY;
		long candidateScore = LONG_MAX;
...
/**
 * Checks whether directional globe-target navigation may be used.
 */
bool GeoscapeState::canNavigateGlobeTargets3DS() const
{
...
/**
 * Finds the nearest visible globe marker in the requested direction.
 *
 * Cursor and returned coordinates use physical screen pixels. Globe
 * marker positions use logical OXCE coordinates and are converted here.
 */
Target *GeoscapeState::snapToGlobeTarget3DS(
        int directionX,
        int directionY,
        int cursorX,
        int cursorY,
        ...
        int &targetY) const
{
                ...
                [&](Target *target)
        {
                ...
                /*
                 * Reuse the normal Geoscape target query so hidden,
                 * undetected, docked, and back-facing targets remain
                 * unavailable to controller snapping.
                 */
                const std::vector<Target*> nearby =
                        _globe->getTargets(
                                x,
                                y,
                                false,
        ...
        for (Base *base :
                *_game->getSavedGame()->getBases())
        {
                addVisibleTarget(base);

                for (Craft *craft :
                        *base->getCrafts())
                {
                        addVisibleTarget(craft);
                }
        }

        for (Ufo *ufo :
                *_game->getSavedGame()->getUfos())
        {
                addVisibleTarget(ufo);
        }
        ...
        for (Waypoint *waypoint :
                *_game->getSavedGame()->
                        getWaypoints())
        {
                addVisibleTarget(waypoint);
        }

        for (MissionSite *site :
                *_game->getSavedGame()->
                        getMissionSites())
        {
                addVisibleTarget(site);
        }

        for (AlienBase *alienBase :
                *_game->getSavedGame()->
                        getAlienBases())
        {
                addVisibleTarget(alienBase);
        }

        /*
         * Ignore whichever target the cursor is already sitting on.
         * Eight logical pixels allows for physical/logical scaling and
         * cursor rounding without making nearby distinct markers too
         * difficult to reach.
         */
        Target *currentTarget = nullptr;
        long long currentDistanceSquared = 64;
        ...
        Target *wrapTarget = nullptr;
        int bestWrapCoordinate = INT_MAX;
        int bestWrapSecondary = INT_MAX;
        long long bestWrapDistance = LLONG_MAX;
        int wrapX = 0;
        int wrapY = 0;
        ...
        for (Target *target : targets)
        {
                ...
                /*
                 * Prefer targets lying ahead of the pressed direction.
                 * Perpendicular distance is weighted so navigation tends
                 * to remain in the same row or column.
                 */
                if (primary > 0)
                {
                ...
                /*
                 * Track the opposite-edge candidate in case no target
                 * lies farther in the requested direction.
                 *
                 * Right wraps to the left edge, Left to the right,
                 * Down to the top, and Up to the bottom.
                 */
                const int wrapCoordinate =
                        static_cast<int>(
                                candidateX) *
                                directionX +
                        static_cast<int>(
...
#ifdef NINTENDO_3DS
/**
 * Moves one step through the six Geoscape time speeds.
 *
 * Positive values move faster and negative values move slower.
 * The selection stops at five seconds and one day instead of wrapping.
 */
void GeoscapeState::stepTimeSpeed3DS(
	int direction)
{
	if (direction == 0)
	{
	 ...
	 * Hard stops prevent:
	 *
	 * ZL on five seconds -> one day
	 * ZR on one day      -> five seconds
	 */
	if (
		nextIndex < 0 ||
		nextIndex >= speedCount)
	{
		return;
	}
