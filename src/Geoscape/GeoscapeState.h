...
private:
	...
	Globe *_globe;
#ifdef NINTENDO_3DS
	Surface *_top3DS = nullptr;
	Surface *_globe3DS = nullptr;

	/*
	 * Owned through State::_surfacesOwned, but excluded
	 * from normal top-screen drawing and input.
	 */
	std::vector<Surface *> _bottomSurfaces3DS;
#endif
...
#ifdef NINTENDO_3DS
	/// Whether an expanded dogfight currently owns D-pad navigation.
	bool canNavigateDogfightControls3DS() const;
	/// Finds the nearest usable dogfight control in a direction.
	bool snapToDogfightControl3DS(
		int directionX,
		int directionY,
		int cursorX,
		int cursorY,
		int &targetX,
		int &targetY) const;
	/// Whether directional globe-target navigation is currently available.
	bool canNavigateGlobeTargets3DS() const;
	/// Steps through Geoscape time speeds without wrapping.
	void stepTimeSpeed3DS(int direction);
	/// Finds the nearest visible globe target in a direction.
	Target *snapToGlobeTarget3DS(
		int directionX,
		int directionY,
		int cursorX,
		int cursorY,
...
#ifdef NINTENDO_3DS
	/// Replaces the logical top image with the centered layout.
	void renderTop3DS(SDL_Surface *surface);
        /// Draws the sidebar directly into the lower-screen source.
        bool renderSidebar3DS(SDL_Surface *surface);
#endif
	void blit() override;
