...
#ifdef NINTENDO_3DS
	/*
	 * These legacy controls are displayed and operated through the
	 * lower screen. Keep them event-active, but prevent top-screen
	 * controller navigation from snapping to their old coordinates.
	 */
...
#ifdef NINTENDO_3DS
	/*
	 * Center only the destination-selection title bar on the
	 * complete 320-pixel geoscape. The rotation and zoom
	 * surfaces retain their original logical coordinates for
	 * bottom-screen input routing.
	 */
	_window->setX(
	        _window->getX() + 32);
	_btnCancel->setX(
...
void SelectDestinationState::handle(Action *action)
{
	 ...
	 * mouse event to the globe, where it would select a destination.
	 */
