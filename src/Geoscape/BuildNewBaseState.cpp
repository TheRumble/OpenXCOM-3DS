...
#ifdef NINTENDO_3DS
BuildNewBaseState *BuildNewBaseState::_active3DS = nullptr;
#endif
...
#ifdef NINTENDO_3DS
	/*
	 * These legacy logical controls are rendered and operated on the
	 * lower screen. They must remain event-active, but the top-screen
	 * menu navigator must not snap to their original coordinates.
	 */
	_btnRotateLeft->setMenuNavigationEnabled(false);
	_btnRotateRight->setMenuNavigationEnabled(false);
	_btnRotateUp->setMenuNavigationEnabled(false);
	_btnRotateDown->setMenuNavigationEnabled(false);
	_btnZoomIn->setMenuNavigationEnabled(false);
	_btnZoomOut->setMenuNavigationEnabled(false);
#endif
...
#ifdef NINTENDO_3DS
	/*
	 * Center only the new-base title bar on the full 320-pixel
	 * 3DS geoscape. The globe controls retain their original
	 * logical coordinates for lower-screen input routing.
	 */
	_window->setX(
	        _window->getX() + 32);
	_btnCancel->setX(
...
#ifdef NINTENDO_3DS
/**
 * Returns the current base-placement state, if one still exists.
 */
BuildNewBaseState *BuildNewBaseState::getActive3DS()
{
	return _active3DS;
}

/**
 * Returns the Geoscape globe used by this placement state.
 */
Globe *BuildNewBaseState::getGlobe3DS() const
{
	return _globe;
}
#endif
...
#ifdef NINTENDO_3DS
	/*
	 * Bottom-screen rotate and zoom events are deliberately aimed at
	 * the legacy control surfaces. Do not also pass the same synthetic
	 * mouse event to the globe, where it would place a base.
	 */
	const SDL_Event *details =
		action ? action->getDetails() : nullptr;

	if (details &&
		(details->type == SDL_MOUSEBUTTONDOWN ||
		 details->type == SDL_MOUSEBUTTONUP) &&
