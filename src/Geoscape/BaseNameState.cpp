...
BaseNameState::BaseNameState(Base *base, Globe *globe, bool first, bool fixedLocation) : _base(base), _globe(globe), _first(first), _fixedLocation(fixedLocation)
{
...
#ifdef NINTENDO_3DS
        /*
         * Center the base-name prompt on the complete 320-pixel
         * geoscape rather than the former 256-pixel globe area.
         */
	...
	/*
	 * Naming a newly placed base is mandatory. Controller Cancel may
	 * dismiss the future software keyboard, but it must not leave this
	 * state or accept an empty name.
	 */
	_edtName->setCancelAllowed(false);
