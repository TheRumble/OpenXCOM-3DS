...
#ifdef NINTENDO_3DS
	/*
	 * Slightly tighter spacing makes room for the additional
	 * platform-specific category.
	 */
	_btnVideo = new TextButton(80, 16, 8, 4);
	_btnAudio = new TextButton(80, 16, 8, 22);
	_btnControls = new TextButton(80, 16, 8, 40);
	_btnGeoscape = new TextButton(80, 16, 8, 58);
	_btnBattlescape = new TextButton(80, 16, 8, 76);
	_btnAdvanced = new TextButton(80, 16, 8, 94);
	_btn3DS = new TextButton(80, 16, 8, 112);
	_btnFolders = new TextButton(80, 16, 8, 130);
#else
...
#ifdef NINTENDO_3DS
	add(_btn3DS, "button", "optionsMenu");
#endif
...
#ifdef NINTENDO_3DS
        /*
         * Nintendo 3DS uses a fixed SDL display mode managed by the
         * platform port. Discard pending desktop display/filter changes
         * rather than recreating the dual-screen video surface.
         */
        const bool smoothFilter =
                Options::newScaleFilter;

        Options::backupDisplay();

        Options::useOpenGL = false;
        Options::useScaleFilter = smoothFilter;
        Options::useHQXFilter = false;
        Options::useXBRZFilter = false;

        Options::newOpenGL = false;
        Options::newScaleFilter = smoothFilter;
...
#ifdef NINTENDO_3DS
		else if (sender == _btn3DS)
		{
			_game->pushState(new Options3DSState(_origin));
		}
#endif
