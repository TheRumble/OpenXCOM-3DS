...
#ifdef NINTENDO_3DS
/* OXCE_3DS_ACTIVE_CONTROLS_STATE */
namespace
{
OptionsControlsState *activeControlsState3DS = nullptr;
}
#endif
...
{
#ifdef NINTENDO_3DS
        /* OXCE_3DS_CONTROLS_CONSTRUCTOR_HOOK */
        activeControlsState3DS = this;
#endif
...
{
#ifdef NINTENDO_3DS
        /* OXCE_3DS_CONTROLS_DESTRUCTOR_HOOK */
        if (activeControlsState3DS == this)
        {
                cancelKeyBinding3DS();
                activeControlsState3DS = nullptr;
        }
#endif
}
...
void OptionsControlsState::updateList()
{
	...
	if (_controlsGeneral[idx].size() > 0)
	{
		...
		_lstControls->setRowSelectable(_offsetGeneralMin, false);
	...
	if (_controlsBase[idx].size() > 0)
	{
		...
		_lstControls->setRowSelectable(_offsetBaseMin, false);
	...
	if (_controlsBattle[idx].size() > 0)
	{
		...
		_lstControls->setRowSelectable(_offsetBattleMin, false);
...
#ifdef NINTENDO_3DS
/* OXCE_3DS_BINDING_STATE_IMPLEMENTATION */
bool OptionsControlsState::isWaitingForKey3DS()
{
        return activeControlsState3DS &&
                activeControlsState3DS->_selected != -1 &&
                activeControlsState3DS->_selKey != nullptr;
}

/* OXCE_3DS_DIRECT_BINDING_ASSIGNMENT */
bool OptionsControlsState::assignKey3DS(SDLKey key)
{
        if (!activeControlsState3DS ||
                activeControlsState3DS->_selected == -1 ||
                !activeControlsState3DS->_selKey)
        {
                return false;
        }

        /* OXCE_3DS_BINDING_MODIFIERS_ASSIGNABLE */
        if (key == SDLK_UNKNOWN)
        {
                return false;
        }

        OptionsControlsState *state =
                activeControlsState3DS;

        *state->_selKey->asKey() = key;

        const std::string name =
                state->ucWords(
                        SDL_GetKeyName(key));

        state->_lstControls->setCellText(
                state->_selected,
                1,
                name);

        state->_lstControls->setCellColor(
                state->_selected,
                0,
                state->_colorNormal);
        ...
        state->_selected = -1;
        state->_selKey = nullptr;

        return true;
}

void OptionsControlsState::cancelKeyBinding3DS()
{
        if (!activeControlsState3DS)
        {
                return;
...
{
#ifdef NINTENDO_3DS
        /* OXCE_3DS_CONTROLS_GROUP_CANCEL */
        cancelKeyBinding3DS();
#endif
	updateList();
}
