...
#ifdef NINTENDO_3DS
    /* OXCE_3DS_BATTLESCAPE_LAYOUT */
    /* Keep gameplay speeds and options in a balanced layout. */
    _txtXcomSpeed->setX(94);
    _txtXcomSpeed->setY(16);
    _slrXcomSpeed->setX(94);
    _slrXcomSpeed->setY(26);

    _txtAlienSpeed->setX(206);
    _txtAlienSpeed->setY(16);
    _slrAlienSpeed->setX(206);
    _slrAlienSpeed->setY(26);

    _txtFireSpeed->setX(94);
    _txtFireSpeed->setY(50);
    _slrFireSpeed->setX(94);
    _slrFireSpeed->setY(60);

    _txtPathPreview->setX(94);
    _txtPathPreview->setY(88);
    _btnArrows->setX(94);
    _btnArrows->setY(98);
    _btnTuCost->setX(94);
    _btnTuCost->setY(116);
    _btnEnergyCost->setX(154);
    _btnEnergyCost->setY(116);

    _txtOptions->setX(206);
    _txtOptions->setY(88);
    _btnTooltips->setX(206);
    _btnTooltips->setY(98);
    _btnDeaths->setX(206);
    _btnDeaths->setY(116);
#endif
...
#ifdef NINTENDO_3DS
    /* OXCE_3DS_BATTLESCAPE_HIDE_DESKTOP_SCROLL */
    /* C-stick movement is configured through the dedicated 3DS page. */
    Options::battleEdgeScroll = SCROLL_NONE;
    Options::battleDragScrollButton = 0;

    _txtEdgeScroll->setVisible(false);
    _cbxEdgeScroll->setVisible(false);
    _cbxEdgeScroll->setMenuNavigationEnabled(false);

    _txtDragScroll->setVisible(false);
    _cbxDragScroll->setVisible(false);
    _cbxDragScroll->setMenuNavigationEnabled(false);

    _txtScrollSpeed->setVisible(false);
    _slrScrollSpeed->setVisible(false);
    _slrScrollSpeed->setMenuNavigationEnabled(false);
#endif
}
