...
OptionsAdvancedState::OptionsAdvancedState(OptionsOrigin origin) : OptionsBaseState(origin)
{
	...
	_lstOptions->onKeyboardPress((ActionHandler)&OptionsAdvancedState::lstOptionsKeyPress, SDLK_LEFT);
	_lstOptions->onKeyboardPress((ActionHandler)&OptionsAdvancedState::lstOptionsKeyPress, SDLK_RIGHT);
	...
	{
#ifdef NINTENDO_3DS
            /* OXCE_3DS_ADVANCED_HIDE_DESKTOP_OPTIONS */
            /* Hide settings that only affect desktop mouse/window behavior. */
            if (optionInfo.id() == "mousewheelSpeed" ||
                optionInfo.id() == "changeValueByMouseWheel" ||
                optionInfo.id() == "FPSInactive" ||
                optionInfo.id() == "vSyncForOpenGL" ||
                optionInfo.id() == "geoDragScrollInvert" ||
                optionInfo.id() == "battleDragScrollInvert")
            {
                    continue;
            }
#endif
...
void OptionsAdvancedState::updateList()
{
	...
	if (_settingsGeneral[idx].size() > 0)
	{
		...
		_lstOptions->setRowSelectable(_offsetGeneralMin, false);
	...
	if (_settingsBase[idx].size() > 0)
	{
		...
		_lstOptions->setRowSelectable(_offsetBaseMin, false);
	...
	if (_settingsBattle[idx].size() > 0)
	{
		...
		_lstOptions->setRowSelectable(_offsetBattleMin, false);
	...
	if (_settingsAI[idx].size() > 0)
	{
		...
		_lstOptions->setRowSelectable(_offsetAIMin, false);
...
void OptionsAdvancedState::addSettings(const std::vector<OptionInfo> &settings)
{
	...
	for (const auto& optionInfo : settings)
	{
		...
		{
			const size_t row = _lstOptions->getLastRowIndex();
			_lstOptions->setRowColor(row, _greyedOutColor);
			_lstOptions->setRowSelectable(row, false);
		}
	}
}
...
/**
 * Changes the selected setting from keyboard/controller navigation.
 * Left decreases integer values; Right increases them. Either
 * direction toggles Boolean settings.
 */
void OptionsAdvancedState::lstOptionsKeyPress(Action *action)
{
	...
	Action click(
		...
		action->getXScale(),
		action->getYScale(),
		action->getTopBlackBand(),
		action->getLeftBlackBand());

	click.setSender(_lstOptions);
	lstOptionsClick(&click);
}
