...
static void setStartupModWorkProgress3DS(
	StartupLoadStage stage,
	int rangeStart,
	int rangeEnd,
...
static unsigned int startupRulesetFilesDone3DS = 0;
static unsigned int startupRulesetFilesTotal3DS = 0;
...
void Mod::loadAll()
{
...
#ifdef NINTENDO_3DS
	startupRulesetFilesDone3DS = 0;
	startupRulesetFilesTotal3DS = 0;
	for (const auto &modRulesets3DS : mods)
	{
		startupRulesetFilesTotal3DS +=
			static_cast<unsigned int>(
				modRulesets3DS.second.size());
	}
#endif
	...
	for (size_t i = 0; _modData.size() > i; ++i)
	{
		...
		setStartupModWorkProgress3DS(
			...
			40,
			static_cast<unsigned int>(i + 1),
			static_cast<unsigned int>(_modData.size()));
#endif
	}
