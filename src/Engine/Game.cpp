...
void Game::run()
{
	...
	while (!_quit)
	{
		...
		// Translate native 3DS controls into SDL events.
#ifdef NINTENDO_3DS
		Input3DS::pump(this);
#endif
...
#ifdef NINTENDO_3DS
			/*
			 * The 3DS SDL backend may repeatedly enqueue SDL_QUIT
			 * once APT requests termination. Stop polling immediately.
			 */
			if (_quit)
			{
				break;
...
#ifdef NINTENDO_3DS
		/*
		 * APT has withdrawn application access. Do not render
		 * another frame after processing SDL_QUIT.
		 */
		if (_quit)
		{
			break;
...
#ifdef NINTENDO_3DS
				if (BottomScreen3DS::getMode() !=
				        BottomScreen3DS::MODE_GEOSCAPE)
				{
				        BottomScreen3DS::captureGameFrame(
				                _screen->getSurface());
				}
#endif
				_fpsCounter->blit(
...
#ifdef NINTENDO_3DS
	/*
	 * On 3DS, SDL_QUIT normally means APT is waiting for this
	 * process to terminate. Do not perform filesystem work here.
	 */
	return;
#else
	Options::save();
