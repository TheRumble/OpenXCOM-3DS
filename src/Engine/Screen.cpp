...
#ifdef NINTENDO_3DS
	_flags |= SDL_DUALSCR | SDL_FITWIDTH;
#endif
...
#ifdef NINTENDO_3DS
	/*
	 * OXCE continues drawing its logical frame with the original
	 * indexed palette. Only the final combined display is true color.
	 */
	_bpp = 8;
#else
...
void Screen::flip()
{
...
#ifdef NINTENDO_3DS
	const int topHeight = _screen->h / 2;


	SDL_Rect topRect = {};
	topRect.x = 0;
	topRect.y = 0;
	topRect.w = static_cast<Uint16>(_screen->w);
	topRect.h = static_cast<Uint16>(topHeight);

	SDL_FillRect(
			_screen,
			&topRect,
			SDL_MapRGB(_screen->format, 0, 0, 0));

	SDL_Rect gameRect = {};
	gameRect.x = static_cast<Sint16>(_leftBlackBand);
	gameRect.y = static_cast<Sint16>(_topBlackBand);
	gameRect.w = static_cast<Uint16>(
			_screen->w - _leftBlackBand - _rightBlackBand);
	gameRect.h = static_cast<Uint16>(
			topHeight - _topBlackBand - _bottomBlackBand);
...
#ifdef NINTENDO_3DS
	/*
	 * The true-color display has no SDL palette to update. The
	 * indexed logical surface already received the new palette in
	 * Screen::setPalette().
	 */
	_numColors = 0;
	_pushPalette = false;
	...
	/* Nintendo 3DS presentation texture filter. */
#ifdef NINTENDO_3DS
	VideoFilter3DS::setSmooth(Options::useScaleFilter);
	VideoFilter3DS::apply();
#endif
    ...
    /*
     * SDL_Flip includes the CPU cache flush, synchronous texture
     * transfer and handoff to SDL's separate Citro3D video thread.
     */
#endif
}
...
void Screen::resetDisplay(bool resetVideo, bool noShaders)
{
...
#ifdef NINTENDO_3DS
	const int displayBpp = 16;
#else
	const int displayBpp = _bpp;
#endif
	...
	if (resetVideo ||
		_screen->format->BitsPerPixel != displayBpp)
	{
		Log(LOG_INFO) << "Attempting to set display to " << width << "x" << height << "x" << displayBpp << "...";
