...
#include "FileMap.h"
#if defined(_WIN32) || defined(NINTENDO_3DS)
#include <malloc.h>
#endif
...
#ifdef NINTENDO_3DS
	_skipMainBlit3DS = other._skipMainBlit3DS;
#endif
}
...
#ifdef NINTENDO_3DS
void Surface::setSkipMainBlit3DS(bool skip)
{
	_skipMainBlit3DS = skip;
}

bool Surface::getSkipMainBlit3DS() const
{
	return _skipMainBlit3DS;
}
#endif
...
/**
 * Builds a compact list of nontransparent runs for each row.
 * The source image is not modified.
 */
bool Surface::buildOpaqueSpans3DS() const
{
	if (!_surface ||
		_surface->format->BitsPerPixel != 8 ||
		_width == 0 ||
		_height == 0)
	{
		return false;
	...
	if (_opaqueSpanRows3DS.size() ==
		expectedRows)
	{
		return true;
	}

	_opaqueSpanRows3DS.clear();
	_opaqueSpans3DS.clear();

	_opaqueSpanRows3DS.resize(
		expectedRows);
	...
	for (int sourceY = 0;
		...
		++sourceY)
	{
		_opaqueSpanRows3DS[sourceY] =
			static_cast<Uint32>(
				_opaqueSpans3DS.size());
...
/**
 * Direct 3DS Battlescape blitter that walks only cached
 * nontransparent source runs.
 */
bool Surface::blitNShadeSpans3DS(
	SurfaceRaw<Uint8> destination,
	int x,
	int y,
	int shade,
	bool half,
	int newBaseColor,
	const GraphSubset *range) const
{
	if (!buildOpaqueSpans3DS())
	{
		return false;
	}

	RenderProfile3DS::ScopedRawBlit
		profileRawBlit3DS;
	...
	int sourceBeginY = 0;
	int sourceEndX = _width;
	int sourceEndY = _height;

	int destinationBeginX =
		x + sourceBeginX;

	int destinationBeginY =
		y;
	...
	if (destinationBeginX < 0)
	{
		destinationBeginX = 0;
	}

	if (destinationBeginY < 0)
	{
		destinationBeginY = 0;
	}
	...
	{
		if (RenderProfile3DS::
			isRawPixelCountingActive())
		{
			RenderProfile3DS::
				recordEmptyRawShader(
					newBaseColor != 0);
		}
	...
	const bool profilePixels3DS =
		RenderProfile3DS::
			isRawPixelCountingActive();

	u64 profileWrittenPixels3DS = 0;
	...
	const Uint8 *shadeTable =
		detail::getStandardShadeTable3DS(
			shade);
	...
	for (int sourceY = sourceBeginY;
		sourceY < sourceEndY;
		++sourceY)
	{
		const Uint32 spanBeginIndex =
			_opaqueSpanRows3DS[
				sourceY];

		const Uint32 spanEndIndex =
			_opaqueSpanRows3DS[
				sourceY + 1];
		...
		for (Uint32 spanIndex =
				spanBeginIndex;
			spanIndex < spanEndIndex;
			spanIndex += 2)
		{
			int spanBegin =
				_opaqueSpans3DS[
	...
	if (profilePixels3DS)
	{
		RenderProfile3DS::recordRawShader(
			static_cast<u64>(
				sourceEndX -
				sourceBeginX) *
			static_cast<u64>(
...
void Surface::blitRaw(SurfaceRaw<Uint8> destSurf, SurfaceRaw<const Uint8> srcSurf, int x, int y, int shade, bool half, int newBaseColor)
{
	...
	RenderProfile3DS::ScopedRawBlit
        ...
        if (!newBaseColor)
        {
                const int sourceWidth = srcSurf.getWidth();
                const int sourceHeight = srcSurf.getHeight();

                const int sourceBeginX =
                        half ? sourceWidth / 2 : 0;

                int destLeft = x + sourceBeginX;
                int destTop = y;
                int destRight = x + sourceWidth;
                int destBottom = y + sourceHeight;

                if (destLeft < 0) destLeft = 0;
                if (destTop < 0) destTop = 0;
                if (destRight > destSurf.getWidth()) destRight = destSurf.getWidth();
                if (destBottom > destSurf.getHeight()) destBottom = destSurf.getHeight();
                ...
                const int copyWidth = destRight - destLeft;
                const int sourceStartX = destLeft - x;
                const int sourceStartY = destTop - y;

                const Uint8 *sourceRow =
                        srcSurf.getBuffer() +
                        sourceStartY * srcSurf.getPitch() +
                        sourceStartX;

                Uint8 *destRow =
                        destSurf.getBuffer() +
                        destTop * destSurf.getPitch() +
                        destLeft;

                const int sourcePitch = srcSurf.getPitch();
                const int destPitch = destSurf.getPitch();

                if (shade == 0)
                {
                        for (int row = destTop; row < destBottom; ++row)
                        {
                                const Uint8 *sourcePixel = sourceRow;
                                Uint8 *destPixel = destRow;

                                int remaining = copyWidth;

                                while (remaining >= 4)
                                {
                                        if (sourcePixel[0]) destPixel[0] = sourcePixel[0];
                                        if (sourcePixel[1]) destPixel[1] = sourcePixel[1];
                                        if (sourcePixel[2]) destPixel[2] = sourcePixel[2];
                                        if (sourcePixel[3]) destPixel[3] = sourcePixel[3];
...
void Surface::blitNShade(SurfaceRaw<Uint8> surface, int x, int y, int shade, GraphSubset range) const
{
	...
	ShaderMove<const Uint8> src(
		this,
		x,
		y);

	ShaderMove<Uint8> dest(
		surface);

	dest.setDomain(
		range);

	ShaderDraw<helper::StandardShade>(
		dest,
		src,
		ShaderScalar(shade));
}
