...
#ifdef NINTENDO_3DS
static void setStartupFileMapProgress3DS(
	StartupLoadStage stage,
	int progress)
{
	StartState::setStartupProgress(stage, progress);
}

class StartupProgressStep3DS
{
private:
	StartupLoadStage _stage;
	int _rangeStart;
	int _rangeEnd;
...
public:
	...
	{
		++_completed;
		StartState::setStartupWorkProgress(
			_stage,
			_rangeStart,
			_rangeEnd,
			_completed,
...
#ifdef NINTENDO_3DS
	unsigned int directoryIndex3DS = 0;
	const StartupLoadStage directoryStage3DS =
		protectedLocation ?
			STARTUP_SCANNING_STANDARD_DATA :
			STARTUP_SCANNING_USER_MODS;
	const int directoryRangeStart3DS =
		protectedLocation ? 5 : 17;
	const int directoryRangeEnd3DS =
		protectedLocation ? 17 : 19;
#endif
	for (auto di = dirlist.begin(); di != dirlist.end(); ++di) {
#ifdef NINTENDO_3DS
		StartupProgressStep3DS progressStep3DS(
			directoryStage3DS,
			directoryRangeStart3DS,
			directoryRangeEnd3DS,
			directoryIndex3DS,
			static_cast<unsigned int>(dirlist.size()));
#endif
		auto mp_basename = *di;
...
void checkModsDependencies() {
#ifdef NINTENDO_3DS
	unsigned int masterWorkDone3DS = 0;
	const unsigned int masterWorkTotal3DS =
		static_cast<unsigned int>(ModsAvailable.size());

	setStartupFileMapProgress3DS(
	...
	unsigned int externalWorkTotal3DS = 0;
	for (auto mri = ModsAvailable.begin();
		mri != ModsAvailable.end();
		++mri)
	{
		const unsigned int resourceCount3DS =
			static_cast<unsigned int>(
				mri->second->modInfo
					.getExternalResourceDirs().size());
		externalWorkTotal3DS +=
			resourceCount3DS == 0 ? 1 : resourceCount3DS;
	}
	unsigned int externalWorkDone3DS = 0;
#endif
	...
	for (auto mri = ModsAvailable.begin(); mri != ModsAvailable.end(); ++mri) {
		...
		for (auto eri = resdirs.begin(); eri != resdirs.end(); ++eri) {
			const bool mapped =
				mapExtResources(mrec, *eri, false);
#ifdef NINTENDO_3DS
			++externalWorkDone3DS;
			StartState::setStartupWorkProgress(
				STARTUP_MAPPING_EXTERNAL_RESOURCES,
