...
{
#ifdef NINTENDO_3DS
	bool mouseEvent3DS = false;
	bool bottomControlEvent3DS = false;

	if (action && action->getDetails())
	{
	...
	if (!_modal)
	{
		for (std::vector<Surface*>::reverse_iterator i = _surfaces.rbegin(); i != _surfaces.rend(); ++i)
		{
			...
			{
#ifdef NINTENDO_3DS
				if ((*i)->getSkipMainBlit3DS() &&
						mouseEvent3DS &&
						!bottomControlEvent3DS)
				{
					/*
					 * Invisible 3DS top-screen UI must not receive real
					 * top-screen hover/click input. For mouse motion only,
					 * feed it a harmless off-panel position so any old
					 * hover state/tooltips can clear.
					 */
					if (action &&
							action->getDetails() &&
							action->getDetails()->type ==
								SDL_MOUSEMOTION)
					{
						SDL_Event *event =
							action->getDetails();
						...
						const Uint16 oldY =
							event->motion.y;
						const Sint16 oldXRel =
							event->motion.xrel;
						const Sint16 oldYRel =
							event->motion.yrel;
						...
						event->motion.x = oldX;
						event->motion.y = oldY;
						event->motion.xrel = oldXRel;
						event->motion.yrel = oldYRel;
					}
