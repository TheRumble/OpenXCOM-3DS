 ...
 */
InteractiveSurface::InteractiveSurface(int width, int height, int x, int y) : Surface(width, height, x, y), _buttonsPressed(0), _in(0), _over(0), _out(0), _isHovered(false), _isFocused(true), _listButton(false), _menuNavigationEnabled(true), _tftdMode(false)
{
}
...
#else
			/*
			 * Disable touchscreen-inherited list-button presses on 3DS.
			 * Physical A-button events still press arrows normally.
			 */
#endif
...
#else
				/*
				 * Disable touchscreen-inherited list-button releases on 3DS.
				 * Physical A-button release events remain unchanged.
				 */
#endif
			}
		}
...
/**
 * Enables or disables controller menu-navigation targeting.
 */
void InteractiveSurface::setMenuNavigationEnabled(bool enabled)
{
	_menuNavigationEnabled = enabled;
}

/**
 * Checks whether controller menu navigation may target this surface.
 */
bool InteractiveSurface::isMenuNavigationEnabled() const
{
