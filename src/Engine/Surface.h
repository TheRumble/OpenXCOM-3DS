...
class Surface
{
	friend class MenuNavigation3DS;
...
protected:
	...
	Uint8 _redraw: 1;
#ifdef NINTENDO_3DS
	Uint8 _skipMainBlit3DS: 1;

	/*
	 * OXCE_3DS_OPAQUE_SPAN_CACHE
	 *
	 * _opaqueSpanRows3DS stores one offset per source row plus
	 * a final sentinel. _opaqueSpans3DS stores alternating
	 * [begin, end) pairs for every nontransparent run.
	 */
	mutable std::vector<Uint32>
		_opaqueSpanRows3DS;

	mutable std::vector<Uint16>
		_opaqueSpans3DS;

	void invalidateOpaqueSpans3DS()
	{
		_opaqueSpanRows3DS.clear();
		_opaqueSpans3DS.clear();
	}

	bool buildOpaqueSpans3DS() const;

	bool blitNShadeSpans3DS(
...
#ifdef NINTENDO_3DS
	/// Sets whether this surface should be skipped by the normal top-screen state blit.
	void setSkipMainBlit3DS(bool skip);
	/// Gets whether this surface should be skipped by the normal top-screen state blit.
	bool getSkipMainBlit3DS() const;
#endif
...
#ifdef NINTENDO_3DS
		/*
		 * OXCE_3DS_INVALIDATE_SPANS_GET_RAW
		 */
...
#ifdef NINTENDO_3DS
		/*
		 * OXCE_3DS_INVALIDATE_SPANS_GET_SURFACE
		 */
		invalidateOpaqueSpans3DS();
#endif
		return _surface.get();
...
#ifdef NINTENDO_3DS
		/*
		 * OXCE_3DS_INVALIDATE_SPANS_GET_BUFFER
		 */
		invalidateOpaqueSpans3DS();
#endif
		return _alignedBuffer.get();
