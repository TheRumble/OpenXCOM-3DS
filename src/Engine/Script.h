...
inline ArgEnum ArgRegisteType()
{
	...
	/*
	 * SDL's Sint32 and the native int type are both signed 32-bit
	 * integers, but they can be different underlying C++ types on
	 * some targets. Treat native 32-bit int as the script integer too.
	 */
	if (std::is_same<T, ScriptInt>::value ||
		(std::is_same<T, int>::value &&
		 sizeof(int) == sizeof(ScriptInt)))
	{
		return ArgInt;
	}
